...
func (k Keeper) GetGenesisNft(ctx sdk.Context, tokenId *sdk.Int) (v1.GenesisNft, bool) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.GenesisNftPrefix)
	var card v1.GenesisNft
	bz := store.Get(tokenId.BigInt().Bytes())
	if len(bz) == 0 {
		return v1.GenesisNft{}, false
	}
	k.cdc.MustUnmarshal(bz, &card)
	return card, true
}

func (k Keeper) SetGenesisNft(ctx sdk.Context, card v1.GenesisNft) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.GenesisNftPrefix)
...
func (k Keeper) GetAllGenesisNfts(ctx sdk.Context) (cards []v1.GenesisNft) {
